...
{
    let serialized = to_item::<T>(value.clone()).unwrap();
    let deserialized = from_item::<T>(serialized).unwrap();
...
#[cfg(test)]
mod from_items {
    use crate::{error::ErrorImpl, from_items, to_attribute_value, AttributeValue, Error};
    use serde_derive::{Deserialize, Serialize};
    ...
    fn same_types() {
        ...
        let users = from_items(items.into()).unwrap();
    ...
    fn wrong_types() {
        ...
        let err = from_items::<Vec<User>>(items.into()).unwrap_err();
