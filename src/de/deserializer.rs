...
impl<'de, 'a> de::Deserializer<'de> for Deserializer {
    ...
    {
        if let AttributeValue::N(n) = self.input {
            DeserializerNumber::from_string(n).deserialize_any(visitor)
        ...
        V: Visitor<'de>,
    {
        ...
        } else if let AttributeValue::M(m) = self.input {
